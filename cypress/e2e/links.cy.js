...
describe('Automation - Working With Links', function () {
  ...
  it.only('Cypress Test Case - Understanding Links and how to work with Links', function () {
    ...
    cy.origin('https://commons.wikimedia.org', () => {
      ...
      cy.get('a').its('length').then((elelength) => {
        ...
        expect(lengthcount).to.equal(697)

        expect(lengthcount).to.be.within(650, 750)
      })
      ...
      cy.get('a[href="/wiki/Commons:Community_portal"]').first().then((txt)=>{
